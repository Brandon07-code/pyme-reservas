...
@section('content')
    ...
    <p class="text-[10px] text-gray-500 mb-2 font-bold uppercase tracking-widest">Resumen Operativo (Clic para filtrar)</p>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <a href="{{ route('empleados.index', array_merge(request()->except('estado'), [])) }}"
        ...
        <a href="{{ route('empleados.index', array_merge(request()->except('estado'), ['estado' => '1'])) }}"
        ...
        <a href="{{ route('empleados.index', array_merge(request()->except('estado'), ['estado' => '0'])) }}"
    ...
    <div class="flex flex-col md:flex-row gap-2 mb-6 items-start md:items-center justify-between">
        <form method="GET" action="{{ route('empleados.index') }}" class="flex gap-2 flex-1">
            ...
            @if($search || $estadoFilter !== null && $estadoFilter !== '') 
                <a href="{{ route('empleados.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-6 rounded shadow text-xs uppercase tracking-wider transition">Limpiar</a> 
            @endif
        </form>
